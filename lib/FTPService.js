...
const c = new Client();
let LOCAL_PATH = '/Users/timo/WebstormProjects/wordpressftpbackup/backups/'
...
const adaptLocalStructure = (path) => {
    fs.readdir(`${process.env.LOCAL_PATH}${path}`, (err, files) => {
        if (err) {
            fs.mkdir(`${LOCAL_PATH}${path}`,() => console.log("Local folder created."));
        }
    });
}
...
const backupDir = (path) => {
    writeLog("Following files detected locally, so not backing up again:");
    console.log(__localBackupPaths)
        ...
        c.list(path,(err, list) => {
            ...
            for (let entry of list) {
                if (entry.type === 'd') {
                    ...
                    if(__localBackupPaths.includes(`${path}/${entry.name}`)){
                        writeLog("Following file wont get backed up: " + `${path}/${entry.name}`)
